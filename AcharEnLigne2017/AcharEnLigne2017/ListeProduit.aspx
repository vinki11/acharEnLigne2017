...
<asp:Content ID="ContentListe" ContentPlaceHolderID="contentPlaceHolderContenu" runat="server">        
        ...
        <asp:SqlDataSource ID="SqlDataSourceProduit" runat="server" 
            ...
            SelectCommand="SELECT [produit_id], [produit_nom], [produit_description], [produit_prix], [produit_image], [categorie_nom], [produit_idCategorie] FROM [produit] INNER JOIN [categorie] ON [produit_idCategorie] = [categorie_id]" 
            UpdateCommand="UPDATE [produit] SET [produit_nom] = @produit_nom, [produit_description] = @produit_description, [produit_prix] = @produit_prix, [produit_image] = @produit_image, [produit_idCategorie] = @produit_idCategorie WHERE [produit_id] = @produit_id"
        ...
        <div id="listeproduits">
            <div class="grille">
                ...
                <asp:GridView ID="GridViewProduits" runat="server" DataSourceID="SqlDataSourceProduit" AutoGenerateEditButton="false" AutoGenerateDeleteButton="false" AutoGenerateColumns="False" HeaderStyle-CssClass="entetegrille" RowStyle-CssClass="lignesgrille" DataKeyNames="produit_id" OnRowCreated="GridViewProduits_RowCreated" OnRowUpdating="GridViewProduits_RowUpdating" OnRowUpdated="GridViewProduits_RowUpdated">
                    <Columns>
                        <%-- Colonne des boutons d'édition --%>
                        <asp:TemplateField>
                            <ItemTemplate>
                                <asp:ImageButton runat="server" ID="imageButtonModifier" imageUrl="Medias/Commun/modifier.png" CommandName="Edit" Width="60px" />
                                <asp:ImageButton runat="server" ID="buttonEffacer" ImageUrl="Medias/Commun/delete.png" CommandArgument='<%# Eval("produit_id") %>' OnClientClick="javascript:return deleteItem(this.name, this.alt);" AlternateText='<%# Eval("produit_id") %>' CommandName="Delete" />
                            </ItemTemplate>
                            <EditItemTemplate>
                                <asp:ImageButton runat="server" ID="imageButtonEnregistrer" imageUrl="Medias/Commun/enregistrer.png" CommandName="Update" ValidationGroup="modif" />
                                <asp:ImageButton runat="server" ID="imageButtonAnnuler" imageUrl="Medias/Commun/annuler.png" CommandName="Cancel" />
                            </EditItemTemplate>
                        </asp:TemplateField>
