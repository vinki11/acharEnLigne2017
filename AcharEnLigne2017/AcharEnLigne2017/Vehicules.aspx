<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Vehicules.aspx.cs" Inherits="Vehicules" %>

<asp:Content ID="ContentCoordonnees" ContentPlaceHolderID="contentPlaceHolderContenu" runat="server">

    <asp:SqlDataSource ID="SqlDataSourceVehicules" runat="server"
        ConnectionString="<%$ ConnectionStrings:ConnectionStringAcharEnLigne2017 %>"
        SelectCommand="SELECT v.[idItem], v.[idModele], v.[idTransmission], v.[km], v.[annee], v.[couleur], v.[nbPorte], v.[statut] FROM [vehicule]">
    </asp:SqlDataSource>

    <!-- Features Wrapper -->
    <div id="features-wrapper" DataSourceID="SqlDataSourceVehicules">
        <div class="container">
            <div class="row">
                <div class="4u">
                    
                    
                    
                    <!-- Box -->
                    <section class="box box-feature">
                        <img src="Medias/Commun/produits/vehicules/nissanTitan2016.jpg" alt="NissanTitan2016" class="image image-full" />
                        <div class="inner">
                            <header>
                                <h2>Produit vedette</h2>
                                <span class="byline">Nissan Titan 2016</span>
                            </header>
                            <p>40 000$</p>
                        </div>
                    </section>
                </div>

                <div class="4u">
                    <!-- Box -->
                    <section class="box box-feature">
                        <img src="Medias/Commun/produits/vehicules/chevroletCorvette2012.jpg" alt="ChevroletCorvette2012" class="image image-full" />
                        <div class="inner">
                            <header>
                                <h2>Meilleure offre</h2>
                                <span class="byline">Chevrolet Corvette 2012</span>
                            </header>
                            <p>34 000$</p>
                        </div>
                    </section>
                </div>

                <div class="4u">
                    <!-- Box -->
                    <section class="box box-feature">
                        <img src="Medias/Commun/produits/vehicules/toyotaCamry2012.jpg" alt="ChevroletCorvette2012" class="image image-full" />
                        <div class="inner">
                            <header>
                                <h2>Aubaine</h2>
                                <span class="byline">Toyota Camry 2012</span>
                            </header>
                            <p>19 000$</p>
                        </div>
                    </section>
                </div>

                <div class="4u">
                    <!-- Box -->
                    <section class="box box-feature">
                        <img src="Medias/Commun/produits/vehicules/toyotaCamry2012.jpg" alt="ChevroletCorvette2012" class="image image-full" />
                        <div class="inner">
                            <header>
                                <h2>Aubaine</h2>
                                <span class="byline">Toyota Camry 2012</span>
                            </header>
                            <p>19 000$</p>
                        </div>
                    </section>
                </div>

            </div>
        </div>
    </div>
</asp:Content>
